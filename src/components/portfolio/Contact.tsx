...
import { useSiteSettings, waLink, waNumber } from "@/hooks/useSiteSettings";
...
export function Contact() {
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    const win = window.open(waLink(settings.whatsapp, text), "_blank", "noopener,noreferrer");

    if (!win) {
      setStatus("error");
      setMsg("Please allow pop-ups so WhatsApp can open, or message me directly.");
      return;
    }
    setStatus("sent");
    setMsg("WhatsApp opened — just tap send and the message lands in my chat.");
    ...
    <section id="contact" className="relative py-28">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_1.2fr]">
          <Reveal>
            <div className="flex h-full flex-col justify-between rounded-3xl p-8 glass neon-border">
              <div className="space-y-5">
                <InfoRow Icon={FiMail} label="Email" value={profile.email} href={`mailto:${profile.email}`} />
                <InfoRow Icon={FiPhone} label="Phone" value={profile.phone} href={`tel:${profile.phone}`} />
                <InfoRow Icon={FiMapPin} label="Location" value={profile.location} />
                <InfoRow Icon={FiGithub} label="GitHub" value="github.com/Jamesgyan" href={profile.github} />
                <InfoRow Icon={FiLinkedin} label="LinkedIn" value="james-gyan-prakash-h" href={profile.linkedin} />
              </div>
          ...
          <Reveal delay={0.05}>
            <form onSubmit={onSubmit} className="rounded-3xl p-8 glass" noValidate>
              ...
              >
                {status === "sent" ? <FiCheck /> : <FiSend className="transition-transform group-hover:translate-x-0.5" />}
                {status === "sending" ? "Sending…" : status === "sent" ? "Sent" : "Send Message"}
              </button>
              {msg && (
                <p
                  className={`mt-4 text-sm ${
                    status === "sent" ? "text-[color:var(--neon-cyan)]" : "text-muted-foreground"
                  }`}
                >
                  {msg}
                </p>
